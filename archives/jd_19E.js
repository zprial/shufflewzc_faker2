      ...
      if (i % 30 == 0) {
        ...
        if (res.data.result.groupInfo?.memberList) {
          ...
          if (memberCount < 30) {
            ...
            groups.push({
              mpin: res.result,
              groupJoinInviteId: groupJoinInviteId,
            });
          ...
          for (var p = 0; p < res.taskVos.length; p++) {
            ...
            switch (task.taskType) {
              ...
              case 0:
                ...
                if (
                  (task.status == 1 && task.taskName !== "去组队分大奖") ||
                  task.status !== 1
                ) {
                  conti = true;
      ...
      if (cookiesArr[i]) {
        ...
        for (let s = 0; s < inviteId.length; s++) {
          ...
          if (res["data"]["bizCode"] === 0) {
            console.log(
              "助力成功,获得：",
              parseFloat(res.data.result.acquiredScore),
              "金币"
            );
...
function promote_collectAtuoScore() {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("promote_collectAutoScore", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function promote_pk_getHomeData() {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("promote_pk_getHomeData", {}),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function getEncryptedPinColor() {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl2("getEncryptedPinColor", {}),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function collectFriendRecordColor(mpin) {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("collectFriendRecordColor", {
        mpin: mpin,
        businessCode: "20136",
        assistType: "2",
        shareSource: 1,
...
function promote_pk_joinGroup(groupJoinInviteId) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("promote_pk_joinGroup", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
