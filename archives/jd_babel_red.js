...
async function main() {
    ...
    if (activeIdInfo) {
        e = JSON.parse(activeIdInfo.activityData.transParam);
        if (e) {
...
function task(functionId, body) {
    let opt = {
        url: `https://api.m.jd.com/client.action?functionId=${functionId}&appid=publicUseApi&t=${new Date().getTime()}&client=wh5&clientVersion=1.0.0&&uuid=${uuidRandom()}&area=21_1911_1916_26729&networkType=&ext={"prstate":"0"}`,
